...
class UserHorse: public HorseRace{
public:
    void MyHorse(){
        cout<< "Your horse SMOKESCREEN  has advanced " << advance << " spaces!!" <<endl;
    } 
};

class Rivalh1: public HorseRace{
public:
    void randHorse1(){
        cout<< "Rival horse SOCKS horse has advanced " << advance << " spaces!!" <<endl;
    } 
};

class Rivalh2: public HorseRace{
public:
    void randHorse2(){
        cout<< "Rival horse PATCHES has advanced " << advance << " spaces!!" <<endl;
    } 
};

class Rivalh3: public HorseRace{
public:
    void randHorse3(){
        cout<< "Rival horse CANDY has advanced " << advance << " spaces!!" <<endl;
    } 
};
...
int main(int argc, char** argv) {
    ...
    //Cards fillDeck(num);
    //Cards prnt(num);
    cout << "The test number is " << temp <<endl;
...
void menu(float& cash){
    ...
    cout << "5" << setw(30) << setfill('*') << "" << "Horse Race" <<endl;
    cout << "6" << setw(30) << setfill('-') << "" << "Exit" <<endl;
    cin >> choice;
    ...
    switch(choice){
        ...
        case 5: horseRacing(cash);break;
        case 6: saveGame(cash);cout << "Exiting program" <<endl;break;
        default: cout << "<Error> invalid entry" <<endl;menu(cash);break;
    }
}

float horseRacing(float& money){
    float bet=0;
    
    UserHorse smokescreen;
    Rivalh1 socks;
    Rivalh2 patches;
    Rivalh3 candy;
    
    HorseRace *userhorse = &smokescreen;
    HorseRace *rival1 = &socks;
    HorseRace *rival2 = &patches;
    HorseRace *rival3 = &candy;
    ...
    cout << "Welcome to 4 horse racing" <<endl;
    cout << "Your horse is Texas Smoke" <<endl;
    cout << "Place your bet on your star horse" <<endl;
    cin >> bet;
    if(bet < money||bet > 0){
        money-=bet;
        cout << "Total dollars $" <<money<<endl; 
        
        while((tot_smoke <=100) || (tot_socks <=100) || (tot_patch <= 100) ||
                (tot_candy <=100)){
            smokeNum=rand()%10+1;
            userhorse->setDistance(smokeNum);
            smokescreen.MyHorse();
            tot_smoke+=smokeNum;
            cout<<"Smokescreen has advanced " << tot_smoke<< " spaces"<<endl;
            if(tot_smoke >=100){
            ...
            sockNum=rand()%10+1;
            rival1->setDistance(sockNum);
            socks.randHorse1();
            tot_socks+=sockNum;
            cout<<"Socks has advanced " << tot_socks<< " spaces"<<endl;
            if(tot_socks >=100 && tot_smoke<= 100 ){
                cout << "YOU HAVE LOST!!!, Socks has won first place"<<endl;
            }
            
            patchNum=rand()%10+1;
            rival2->setDistance(patchNum);
            patches.randHorse2();
            tot_patch+=patchNum;
            cout<<"Patches has advanced " << tot_patch<< " spaces"<<endl;
            if(tot_patch >=100 && tot_smoke<=100 && tot_socks<=100){
                cout << "YOU HAVE LOST!!!, Patches has won first place"<<endl;
            }
            
            candyNum=rand()%10+1;
            rival3->setDistance(candyNum);
            candy.randHorse3();
            tot_candy+=candyNum;
            cout<<"Candy has advanced " << tot_candy<< " spaces"<<endl;
            if(tot_candy >=100 && tot_smoke<=100 && tot_socks<=100
                    && tot_patch<=100){
                cout << "YOU HAVE LOST!!!, Candy has won first place"<<endl;
            }
            cout<<endl<<endl<<endl<<endl<<endl;
        }
        cout<<"Total dollars $"<<money<<endl<<endl;
        char choice;
...
float keno(float & money){
    ...
    money+=bet;
    cout << "You have a total of " << money << " dollars"<<endl;
    keno(money);
}
