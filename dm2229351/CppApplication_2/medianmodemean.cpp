...
int main(int argc, char** argv) {
    ...
    cout << "\nThe max freq in the array = " << maxFreq(brray, SIZE);
    cout << "\nThe mean in the array = " << mean(brray, SIZE);
    cout << "\nThe median in the array = " << median(brray, SIZE);
    int *mode= numode(brray, SIZE, maxFreq(brray, SIZE));
    cout << "\nThe number of modes is " << mode[0];
    cout << "\nThe max frequency is " << mode[1];
    cout << "\nThe modes is/are "; 
    for(int i=2;i<mode[0]+2;i++) cout << mode[i] << " ";
    destroy(array);
    destroy(mode);
    return 0;
}
